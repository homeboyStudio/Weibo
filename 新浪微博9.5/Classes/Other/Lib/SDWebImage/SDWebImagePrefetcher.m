...
@property (assign, nonatomic) int requestedCount;
@property (assign, nonatomic) int skippedCount;
...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            NSLog(@"Prefetched %d out of %lu", self.finishedCount, (unsigned long)self.prefetchURLs.count);
        }
        else
        {
            NSLog(@"Prefetched %d out of %lu (Failed)", self.finishedCount, (unsigned long)[self.prefetchURLs count]);
...
- (void)reportStatus
{
    unsigned long total = [self.prefetchURLs count];
    NSLog(@"Finished prefetching (%lu successful, %d skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
